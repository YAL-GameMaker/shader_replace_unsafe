function scr_oldsh() {
    return [ @'float4 vec4(float x0, float x1, float x2, float x3)
{
...
uniform float4 _gm_AmbientColour : register(c0);
uniform float _gm_FogStart : register(c1);
uniform bool _gm_LightingEnabled : register(c2);
uniform float4 _gm_Lights_Colour[8] : register(c3);
uniform float4 _gm_Lights_Direction[8] : register(c11);
uniform float4 _gm_Lights_PosRange[8] : register(c19);
uniform float4x4 _gm_Matrices[5] : register(c27);
uniform float _gm_RcpFogRange : register(c47);
uniform bool _gm_VS_FogEnabled : register(c48);
...
{
{
float4 _c = (_v_vColour * gl_texture2D(texture__gm_BaseTexture, sampler__gm_BaseTexture, _v_vTexcoord));
(_c.xyz = (float3(1.0, 1.0, 1.0) - _c.xyz));
_c.x = 1.0;
(gl_Color[0] = _c);
}
}
